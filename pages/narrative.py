...
# Anda bisa memilih indikator yang berbeda di sini
...
# --- MEMBUAT FIGUR ---
# (Logika pembuatan figur sama persis seperti kode final kita sebelumnya)
if not df_map_latest.empty:
    fig_map = px.choropleth(df_map_latest, geojson=geojson_data, locations='iso_alpha', featureidkey="properties.ISO_A3", color='LogValue', color_continuous_scale="Blues", hover_name='Country', hover_data={'Value': ':.2f', 'Year': True, 'LogValue': False}, labels={'LogValue': 'Value (Log Scale)'})
    fig_map.update_layout(margin={"r":0, "t":0, "l":0, "b":0})
else:
    fig_map = go.Figure().add_annotation(text="Data tidak tersedia", show_arrow=False)

if not df_coverage_latest.empty:
    fig_coverage = px.bar(df_coverage_latest.sort_values('Value', ascending=True), x='Value', y='Country', orientation='h', labels={'Value': '% Populasi Terjangkau', 'Country': ''}, text='Value')
